...
class VeriKalitesiRaporu:
    ham_gozlem: int
    temiz_gozlem: int
    olcek_duzeltilen_pazarlar: dict[str, float]   # {'B Pazarı': 100.0}
    mf_kirpilan: int
    iade_gozlem: int
    ilk_satis_oncesi_atilan: int
    kisa_seriler: dict[tuple[str, str, str], int]  # anahtar → dolu ay sayısı

def mf_olcek_tespit(df) -> dict[str, float]:      # pazar → 1.0 veya 100.0
def temizle(df) -> tuple[pd.DataFrame, VeriKalitesiRaporu]
```
...
| Test | Doğruladığı |
|---|---|
| `test_mf_olcek_tespiti_yalnizca_b_pazarini_yuzde_kabul_eder` | 🚨 `{'B Pazarı': 100.0}`, diğer üçü `1.0` — bulgular §2.3 medyanları |
...
| `test_ilk_satistan_onceki_aylar_atiliyor` | Sentetik seri: ilk pozitiften önceki satırlar yok, sonraki sıfırlar duruyor (V5) |
...
⚠️ `mf_olcek_tespit` yalnız `mf_oran > 0` olan gözlemlerin medyanına bakar; sıfırlar
